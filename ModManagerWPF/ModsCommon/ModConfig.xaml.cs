	...
	public class ConfigSettings
	{
		...
		public ConfigSettings(string path)
		{
			...
			foreach (ConfigSchemaGroup group in schema.Groups)
			{
				...
				foreach (ConfigSchemaParentProperty prop in group.Properties)
					if (!configINI[group.Name].ContainsKey(prop.Name))
					{
						...
						if (prop.ChildProperties.Count > 0)
							foreach (ConfigSchemaProperty child in prop.ChildProperties)
								if (!configINI[group.Name].ContainsKey(child.Name))
									configINI[group.Name].Add(child.Name, child.DefaultValue);
					}
			}
		}
