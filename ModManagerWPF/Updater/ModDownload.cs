...
namespace SAModManager.Updater
{
    ...
    public class ModDownloadWPF
    {
        ...
        public ModDownloadWPF(ModInfo info, string folder, string url, string changes, long size)
        {
            Info = info;
        ...
        public async Task Download(WebClient client, string updatePath)
        {
                ...
                case ModDownloadType.Modular:
                    {
                        ...
                        foreach (ModManifestDiff i in newEntries)
                        {
                                ...
                                !i.Current.Checksum.Equals(ModManifestGenerator.GetFileHash(filePath), StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                await client.DownloadFileTaskAsync(new Uri(uri, i.Current.FilePath), filePath);
                        ...
                        await client.DownloadFileTaskAsync(new Uri(uri, "mod.manifest"), Path.Combine(tempDir, "mod.manifest"));
